...
> **✅ Implementation status (2026-07-15):** Phases 0–3 of the rollout in §11 are **built and merged** (green `pnpm build`). Delivered: the risk-score model (`ProctorFlag`, weighted scoring, server-side warn/terminate), browser-signal upgrades (multi-monitor + paste-content logging), the pre-exam **system-check + consent + identity** wizard, on-device **MediaPipe face detection** with adaptive snapshot evidence, private (signed-URL) evidence storage, and a **reviewer dashboard** (`/admin/contest/:id/proctoring`). Opt-in per contest via the new `mediaProctoring` config — existing contests are unaffected. **Not yet built (§11 Phases 4–6):** audio VAD, `getDisplayMedia` full recording, object/phone detection (COCO-SSD), ID-face-match, code-similarity. Deployment (§10) is unchanged guidance. The sections below are the original plan, kept as the reference spec.
