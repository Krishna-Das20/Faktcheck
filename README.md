...
- **Audio + object detection** (opt-in) — on-device Web Audio voice-activity detection flags sustained background voices; TensorFlow.js COCO-SSD flags phones/books/laptops in frame (the catastrophic `OBJECT_DETECTED` signal) with an evidence snapshot
- **Reviewer dashboard** (`/admin/contest/:id/proctoring`) — candidates ranked by risk score with flag counts, per-candidate flag timeline with evidence thumbnails, identity photo, consent record, and **clear / confirm-violation / void-attempt** decision actions (voiding zeroes the score but keeps the record)
- **Code-similarity scan** (`/admin/contest/:id/similarity`) — reviewer-triggered structural-fingerprint (simplified MOSS: k-gram + winnowing + Jaccard) comparison of best submissions per problem/language, surfacing similar pairs for human review
